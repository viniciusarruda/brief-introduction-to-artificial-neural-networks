...
class FlappyBird:
	...
	def run(self, birds, best_info):
		...
		while True:
			...
			self._show_statistcs(birds, best_info)
			...
			dist_wall = self.middle_wall_x - birds[0].bird[0] - 40

			for bird in birds:
				bird.update(self.upRect, self.downRect, self.middle_wall_x, self.middle_gap_y, self.travelled_dist, self.current_speed, self.generation)

			self.travelled_dist += self.current_speed

			pg.display.update()

			if all([bird.out_of_screen for bird in birds]):
				return


	def _show_statistcs(self, birds, best_info):
		...
		_show_header('GLOBAL TIME ELAPSED: {}'.format(_time_format(self.start_global_time)))
		_show_header('GENERATION TIME ELAPSED: {}'.format(_time_format(self.start_time)))
		_show_header('SPEED: {}'.format(self.current_speed))
		_show_header('GENERATION: {}'.format(self.generation))
		
		_blit('BIRD ID : STATE : TRAVELLED DISTANCE', FlappyBird._WHITE)

		for bird in birds:
			color, text = (FlappyBird._RED, 'DEAD') if bird.dead else (FlappyBird._GREEN, 'ALIVE')
			_show_bird_stat(bird.bird_num, text, color, bird.current_traveled_distance)

		if best_info is not None:

			self.height += self.vertical_space
			_show_footer('GLOBAL BEST BIRD')
			_show_footer('ID: {}'.format(best_info[0]))
			_show_footer('GENERATION: {}'.format(best_info[4]))
			_show_footer('FITNESS: {}'.format(best_info[2]))
